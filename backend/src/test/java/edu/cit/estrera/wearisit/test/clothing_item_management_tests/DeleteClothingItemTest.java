...
class DeleteClothingItemTest {
    ...
    void deleteClothingItem_Success() {
        ...
        doNothing().when(clothingItemRepository).delete(item);
    ...
    @Test
    void deleteClothingItem_ThrowsException_WhenItemNotFound() {
        when(securityUtil.getCurrentUser()).thenReturn(user);
        when(clothingItemRepository.findByIdAndUser_Id(999L, 1L))
                .thenReturn(Optional.empty());

        assertThatThrownBy(() -> clothingItemService.deleteClothingItem(999L))
    ...
    @Test
    void deleteClothingItem_ThrowsException_WhenItemBelongsToAnotherUser() {
        ...
        assertThatThrownBy(() -> clothingItemService.deleteClothingItem(1L))
                .isInstanceOf(ApiException.class)
                .satisfies(ex -> {
                    ApiException apiEx = (ApiException) ex;
                    assertEquals(ErrorCode.ITEM_001, apiEx.getErrorCode());
    ...
    @Test
    void deleteClothingItem_DeletesEvenWithCategoriesAndTags() {
        when(securityUtil.getCurrentUser()).thenReturn(user);
        when(clothingItemRepository.findByIdAndUser_Id(1L, 1L))
                .thenReturn(Optional.of(item));
        doNothing().when(clothingItemRepository).delete(item);
